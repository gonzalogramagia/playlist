...
export default function ImportModal() {
  ...
  const t = {
    ...
    dropLabel: isEnglish
      ...
      : "Arrastra el archivo JSON aquí o click para subir",
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
      <div className="bg-white dark:bg-zinc-900 w-full max-w-md rounded-xl shadow-2xl border border-zinc-200 dark:border-zinc-800 flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="flex items-center justify-between p-4 border-b border-zinc-100 dark:border-zinc-800">
          ...
          <Link
            to="/"
            className="p-1 rounded-full hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors cursor-pointer"
          >
        ...
        <div className="p-6 space-y-6">
          ...
          <div
            ...
            className={`border-2 border-dashed rounded-xl p-8 flex flex-col items-center justify-center text-center cursor-pointer transition-colors ${
              file
                ? "border-[#6866D6] bg-[#6866D6]/10 dark:bg-[#6866D6]/20"
          ...
          >
            ...
            {file ? (
              ...
              <>
                ...
                <span className="text-sm text-zinc-500 max-w-[200px]">
                  {t.dropLabel}
                </span>
              </>
            )}
          ...
          {previewData && !error && (
            <div className="bg-zinc-50 dark:bg-zinc-800/50 rounded-lg p-4 animate-in fade-in slide-in-from-top-4 duration-300">
              <p className="text-sm text-zinc-700 dark:text-zinc-300 font-medium">
                {t.videosFound}{" "}
                <span className="text-[#6866D6] dark:text-[#6866D6]">
                  {previewData.length}
                </span>
              </p>
              <div className="mt-2 flex flex-wrap gap-1 max-h-20 overflow-y-auto">
                {Array.from(new Set(previewData.flatMap((v) => v.tags)))
                  .slice(0, 10)
                  .map((tag) => (
                    <span
                      key={tag}
                      className="text-xs bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 px-2 py-1 rounded-full text-zinc-600 dark:text-zinc-400"
                    >
                      {tag}
                    </span>
                  ))}
